...
test_discovery_counts_and_depth() {
    ...
    assert_equals "3" "$count" "repo count" || return 1
    ...
    assert_contains "$(cat "$discover_file")" "." "contains workspace root" || return 1
...
test_yaml_schema_and_paths() {
    ...
    assert_equals "3" "$repo_count" "repos length" || return 1
...
test_merge_preserves_annotations() {
    ...
    repo_count=$(yq eval '.repos | length' "$yaml_path")
    assert_equals "4" "$repo_count" "repo count after merge" || return 1
    return 0
}
